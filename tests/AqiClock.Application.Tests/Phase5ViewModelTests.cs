...
public sealed class Phase5ViewModelTests
{
    ...
    [Fact]
    public async Task InactiveAccountSyncFailureIsNotReportedAsConnectivity()
    {
        var inactive = new SessionState(Guid.NewGuid(), "teacher@example.test", UserRole.Teacher, false, false, RoleConfirmed: true);
        var vm = CreateSignInViewModel(new SessionStub(current: inactive), new SyncStub(new InvalidOperationException("profile inactive")));
        vm.Email = "teacher@example.test"; vm.Password = "correct-password";

        await vm.SignInCommand.ExecuteAsync(null);

        Assert.Equal("Your account is inactive; contact an administrator.", vm.ErrorMessage);
    }
    ...
    private sealed class GatewayStub : ISupabaseGateway
    {
        ...
        public Task SaveTimetableAsync(TimetableRow timetable, IReadOnlyList<PeriodRow> periods, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task SaveWeekScheduleRowAsync(int weekday, Guid? audienceClassId, Guid? timetableId, CancellationToken cancellationToken = default) => throw new NotSupportedException();
        public Task DeleteWeekScheduleRowAsync(int weekday, Guid audienceClassId, CancellationToken cancellationToken = default) => throw new NotSupportedException();
