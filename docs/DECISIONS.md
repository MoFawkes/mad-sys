...
**Accepted** 2026-07-28.
The mobile student session persists its selected classes and independent Naseehah AM/PM choices locally. Owner decision 2026-08-01 supersedes the former shared-PC desktop clause: desktop student installations are also one student per machine and persist the same choices in the local cache. Owner decision 2026-08-11 removes the per-period class-tag editor: class-specific timetables are selected through class-specific week-schedule rows, while the default row remains school-wide. The period-class schema and matching engine remain for compatibility, but no product UI creates those tags and production rows must be empty. Notifications use the resolved class-specific schedule. Rejected: retaining per-launch selection on personal desktop installs and maintaining two competing class-targeting mechanisms.
...
**Accepted** 2026-08-01.
Admin and Settings leave their XAML `CenterOwner` startup location untouched until a usable saved placement exists. Saved rectangles select the nearest live monitor through `MonitorFromRect`, so an unplugged display falls back safely, and `GetMonitorInfo` supplies that monitor's work area. Because Win32 reports the work area in physical pixels while WPF placement uses device-independent units, the controller uses `GetDpiForMonitor(MDT_EFFECTIVE_DPI)` for the selected target and converts its work rectangle before applying the shared pure clamp. Resolution failures are logged and fall back atomically to the XAML defaults. No Windows Forms dependency is introduced. Multi-monitor layouts with any non-100% scaling remain a manual acceptance item: the saved WPF rectangle is in DIUs but `MonitorFromRect` expects physical pixels, and scaling an absolute virtual-desktop work-area coordinate by one monitor's factor is not generally valid. Single-monitor layouts at any scale and uniform-100% multi-monitor layouts are supported. Display topology cannot be represented by the unit-test environment; clamp arithmetic, null placement, and corrupt-placement fallback remain automated.
...
**Accepted** 2026-08-18.
When a device selects more than one class, weekday resolution returns every matching class-specific assignment and merges their valid periods by start time and sort order. The default assignment is used only when no selected class has a matching row. Each merged period retains its source class for conditional clock labels; a timetable shared by multiple selected classes is emitted once without a misleading single-class label. Notifications that share a trigger instant and kind are presented as one composite notification while their underlying event identities remain stable. The generator release restores the warning deferred in v0.14.0 for concurrently selectable generated class timetables: overlapping minutes warn only when the periods differ by name, start, or end, with no duration floor. Identical clock labels remain silent even though their timetable-scoped period identifiers differ. Rejected: silently choosing the lowest class identifier, emitting stacked notifications, and suppressing short clashes with an arbitrary duration threshold.
...
## ADR-028: Materialise a rolling window of dated timetable variants
**Accepted** 2026-08-26; **Superseded** 2026-08-26 by ADR-029.
ADR-027 keeps expanded `periods` as the wire format for already-shipped clients, but Maghrib moves on essentially every date and mobile schedules seven days ahead. Rewriting one live timetable daily would therefore give future notifications stale anchors, while runtime client reflow would make every installed v0.13.x/v0.14.0 client incompatible. Clients also pull whole tables rather than deltas, so materialising dates indefinitely would replicate unbounded variants to every device.

Materialise dated variant timetables for a rolling 30-day window and pin them through `date_overrides`. Add nullable `audience_class_id` to `date_overrides` and enforce `unique nulls not distinct (org_id, date, audience_class_id)`, matching the audience-aware `week_schedule` shape, so AM and PM audiences can each select their variant on the same date. A missing variant deliberately falls back to the base timetable: the degraded state is slightly stale prayer timing, not an empty or broken schedule. Compatibility with two audience rows on one date must be proven using an actual v0.14.0 client before downstream UI work proceeds.

Maintenance is unattended. A Cloudflare Worker Cron Trigger runs daily at 02:17 UTC and posts directly to the PostgREST endpoint for `public.run_generator_maintenance(p_org_id uuid)` using a dedicated automation secret. That function is granted only to `service_role`; it cannot use the normal admin gate because service-key calls have no `auth.uid()` profile from which `private.is_admin()` or `private.current_org_id()` can resolve authorization or organization. The desktop redundancy path is `public.admin_extend_generator_window()`, which remains admin-gated and derives the organization from the signed-in caller. Both delegate to `private.generator_maintenance(p_org_id uuid)`.

The shared maintenance body runs as one transaction under an organization-scoped advisory transaction lock. It derives today from `timezone(organizations.timezone, now())::date`, materialises through 30 days ahead, prunes variants before today, verifies the resulting horizon, and writes `generator_maintenance_runs` evidence consumed by the Admin window-status panel. Every generated-period write sets the transaction-local `aqi.generator_write` flag required by the generated-period guard. The Worker does not orchestrate materialisation and pruning as separate calls, and no Supabase Edge Function is introduced.

Pruning is load-bearing because all clients pull complete tables: expired variants, their periods, and their override rows must be removed rather than accumulating on every cache. The desktop also extends on admin sign-in and sync, offers an explicit “extend now” action, and warns when fewer than 14 days remain, providing both redundancy and a visible scheduler-failure channel.

Rejected: runtime reflow on clients, because it contradicts ADR-027 and silently fails on installed versions; regenerating only the live timetable in place, because mobile needs seven future days; desktop-only triggering, because the window would lapse whenever no admin PC runs; `pg_cron`, because it is not provisioned and the tier review is outside v0.15.0; and splitting maintenance across Worker calls, because partial execution would leave a state the next run must repair.

The required compatibility test disproved this decision on the day it was accepted. Manual test 4 used a real v0.14.0 build at commit `0889e448`, selected the `MANUAL TEST AM` audience, and observed `PM AUDIENCE WRONG` while the client reported “Synced · just now”. Shipped clients resolve `date_overrides` by `(org, date)` and take whichever duplicate row wins enumeration order; they do not resolve by audience. The mobile repository has the same limitation. A nullable `audience_class_id` would therefore be schema-readable but behaviorally incompatible: an old AM client could silently render the PM day. The proposed `date_overrides` migration, materialise/prune RPCs, rolling window, horizon check, and pruning path are cancelled rather than worked around in client code.

## ADR-029: Regenerate generated timetable periods in place each day
**Accepted** 2026-08-26.
Regenerate each generated timetable's ordinary `periods` rows in place once a day for the organisation's local date. Do not create dated timetable variants, do not involve `date_overrides`, and do not maintain or prune a future window. Old clients continue reading the periods of their assigned timetable exactly as they always have, so audience isolation remains the existing `week_schedule` and timetable assignment behavior rather than a new compatibility assumption.

The seven-day mobile notification horizon is the accepted trade-off rather than a reason to corrupt audience selection. Desktop only looks ahead far enough to name the next period, whose first PM lesson starts before any moving prayer anchor. Mobile is the sole consumer of future-dated accuracy: it schedules seven days from the timetable as regenerated today, so a device that stops reconciling can drift by roughly two minutes per day since its last sync. A device reconciling daily gets today exactly right. If that residual later proves material, future variants belong in a new table that old clients never read, with old clients falling back to the daily-regenerated base.

Maintenance remains unattended. A Cloudflare Worker Cron Trigger runs daily at 02:17 UTC and posts directly to `/rest/v1/rpc/run_generator_maintenance` using a dedicated automation secret. `public.run_generator_maintenance(p_org_id uuid)` is granted to `service_role` only; it takes the organisation explicitly because service-key calls have no `auth.uid()` profile from which the admin helpers can derive one. `public.admin_regenerate_generated_timetables()` is the admin-gated desktop redundancy path. Both delegate to `private.generator_maintenance(p_org_id uuid)`.

The shared body runs in one transaction under an organisation-scoped advisory transaction lock. It derives the target date with `timezone(organizations.timezone, now())::date`, sets the transaction-local `aqi.generator_write` flag, re-expands every generated timetable for that date, replaces its periods, and records the run in `generator_maintenance_runs`. The run record carries the regenerated date and number of timetables written, not a materialisation horizon. Admin sign-in, sync, and an explicit “regenerate now” action provide redundancy; the Admin panel shows the last successful run and warns after 48 hours.

Idempotency is the primary correctness property. Running maintenance twice for the same date and inputs must leave identical rows and identifiers and must not create a duplicate run record or spurious audit writes. ADR-027's deterministic identifiers make that structural; a second-run diff indicates an upstream derivation error and must not be hidden by the RPC.
