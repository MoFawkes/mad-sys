...
- **Interruption reflow**: the desktop editor computes insert/shift candidates in memory and validates every resulting boundary before mutating its observable period list. A successful operation closes the seam immediately before the shifted range and uses the existing atomic whole-list timetable RPC; expanded `periods` remain the only wire format, so mobile and older caches need no schema change.
...
The schedule engine is a pure function of `(schedule data + viewer class ids, date)`. Viewer class ids live on `ScheduleSnapshot`, which pre-resolves the seven week-schedule entries so nested lookahead calculations cannot accidentally lose the audience.
