...
import { at, id, localDate, monday, normalDay, override, period, timetable, tuesday, week, weekOf } from '../test/testData';
...
describe('findNextPeriod', () => {
  ...
  it('skips days with no valid periods', () => {
    ...
    const snapshot: ScheduleSnapshot = { timetables: [normal, broken], weekSchedule: week([0, normal.id], [1, broken.id], [2, normal.id]), dateOverrides: [] };
  ...
  it('uses a future override timetable', () => {
    ...
    const snapshot: ScheduleSnapshot = { timetables: [normal, exam], weekSchedule: week([0, normal.id]), dateOverrides: [override(tuesday, exam.id)] };
