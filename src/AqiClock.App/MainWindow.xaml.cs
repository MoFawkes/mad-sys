...
public partial class MainWindow : Window
{
    ...
    private void ApplyMode() { bool compact = _viewModel.DisplayMode == DisplayMode.Compact; NormalLayout.Visibility = compact ? Visibility.Collapsed : Visibility.Visible; CompactLayout.Visibility = compact ? Visibility.Visible : Visibility.Collapsed; AnnouncementPanel.Visibility = _viewModel.IsAnnouncementsOpen && !compact ? Visibility.Visible : Visibility.Collapsed; Topmost = _viewModel.AlwaysOnTop; RestorePlacement(compact ? _settings.Current.CompactPlacement : _settings.Current.NormalPlacement); }
    private void RestorePlacement(WindowPlacement? placement) { if (placement is null) return; Rect work = SystemParameters.WorkArea; if (placement.Left < work.Right && placement.Top < work.Bottom && placement.Left + placement.Width > work.Left && placement.Top + placement.Height > work.Top) { Left = placement.Left; Top = placement.Top; } else { WindowStartupLocation = WindowStartupLocation.CenterScreen; } }
